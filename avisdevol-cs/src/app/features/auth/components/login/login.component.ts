  ...
  private handleQueryParams(): void {
    this._activatedRoute.queryParams.subscribe(params => {
      ...
      if (params['expired'] === 'true') {
        this._notificationService.displayWarning(
          this._translateService.instant('COMMONS.CONNEXIONPAGE.SESSION_EXPIRED')
        );
      }
      
      if (params['error'] === 'invalid_token') {
        this._notificationService.displayError(
          this._translateService.instant('COMMONS.CONNEXIONPAGE.INVALID_TOKEN')
        );
      }
      
      if (params['returnUrl']) {
        this._notificationService.displayInfo(
          this._translateService.instant('COMMONS.CONNEXIONPAGE.LOGIN_REQUIRED')
        );
      }
    });
  }
...
public connexion() {
  const credentials = this.computeCredentials();
    this._authenticateService.login(credentials).pipe(
      tap(response => {
